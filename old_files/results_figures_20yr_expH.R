################################################################################
##  results_figures_20yr.R: Compiles Bayesian output and makes figures for the supplemental analysis of e^H and compositonal differences between treatment and control plots for datasets cut off at 20 years.
##
##  Author: Kimberly La Pierre
##  Date created: February 10, 2018
##  See https://github.com/klapierre/Converge_Diverge/blob/master/core%20data%20paper_bayesian%20results_figures_sig%20test_expinteractions_20yr_expHPC.R for full history.
################################################################################
...
rawData2<- rawData%>%
  ...
  summarise(mean_mean=mean(mean_change), std_mean=sd(mean_change), mean_rich=mean(expH_PC), std_rich=sd(expH_PC)) #to backtransform

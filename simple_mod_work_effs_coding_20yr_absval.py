...
data_name = "timestdbytrt_20yr_abs"
...
corre_data.loc[:,'s_change_std'] = std_fun(corre_data['S_lnRR_abs'])
...
for i in range(1,B_MC.shape[1]):
    ...
    plt.savefig('absmc_figs_timestd_noninf/{}.png'.format(i))
    plt.close()
    tempBSPC = B_SPC[:,i-1,:]
    ...
    plt.plot(tempx, tempy, 'k-')
    plt.title(tempdf['treatment_ID'].unique())
    plt.savefig('abslnrr_figs_timestd_noninf/{}.png'.format(i))
    plt.close()

################################################################################
##  timestd_lnRR.py: Bayesian analysis examining differences in the absolute value of richness and composition for datasets cutoff at 20 years.
##
##  Author: Nate Lemoine
...
data_name = "timestdbytrt_20yr_lnRR"
...
corre_data.loc[:,'s_change_std'] = std_fun(corre_data['S_lnRR'])
...
for i in range(1,B_MC.shape[1]):
    ...
    plt.savefig('lnrr_mc_figs_timestd_noninf/{}.png'.format(i))
    plt.close()
    tempBSPC = B_SPC[:,i-1,:]
    ...
    plt.plot(tempx, tempy, 'k-')
    plt.title(tempdf['treatment_ID'].unique())
    plt.savefig('lnrr_slnrr_figs_timestd_noninf/{}.png'.format(i))
    plt.close()

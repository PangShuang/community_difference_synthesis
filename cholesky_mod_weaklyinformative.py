################################################################################
##  simple_mod_cholesky_weaklyinformative.py: Bayesian analysis with weakly informative priors.
##
##  Author: Nate Lemoine
...
model{
	...
	for(m in 1:2){
		L_Omega_B[m] ~ lkj_corr_cholesky(2.0);
		to_vector(effs_B[m]) ~ normal(0,1);
		L_Omega_G[m] ~ lkj_corr_cholesky(2.0);
		to_vector(effs_G[m]) ~ normal(0,1);
		to_vector(U[m]) ~ normal(0,to_vector(U_sigma[m]));
		to_vector(U_sigma[m]) ~ gamma(0.5, 0.5);
		to_vector(D[m]) ~ normal(0, to_vector(D_sigma[m]));
		to_vector(D_sigma[m]) ~ gamma(0.5, 0.5);
	}

	L_Omega_Y ~ lkj_corr_cholesky(2.0);
}
"""
simple_comp = StanModel(model_code=simple_model)

with open('simple_cholesky_weaklyinformative.pkl', 'wb') as f:

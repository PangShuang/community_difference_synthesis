################################################################################
##  simple_mod_work_effs_coding_20yr_expH.py: Bayesian analysis examining differences in e^H and composition for datasets cutoff at 20 years.
##
##  Author: Nate Lemoine
...
# nature_cholesky_mod_noninformative
# nature_cholesky_mod_N01
# nature_cholesky_mod_normal-gamma
model_name = "nature_cholesky_mod_noninformative"
# input dataframe name here WITHOUT the .csv extension
data_name = "timestdbytrt_20yr_eH"
...
corre_data.loc[:,'s_change_std'] = std_fun(corre_data['expH_lnRR
'])
...
for i in range(1,B_MC.shape[1]):
    ...
    plt.savefig('eHmc_figs_timestd_noninf/{}.png'.format(i))
    plt.close()
    tempBSPC = B_SPC[:,i-1,:]
    ...
    plt.plot(tempx, tempy, 'k-')
    plt.title(tempdf['treatment_ID'].unique())
    plt.savefig('eHlnrr_figs_timestd_noninf/{}.png'.format(i))
    plt.close()
